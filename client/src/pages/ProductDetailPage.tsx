...
export const ProductDetailPage: React.FC = () => {
  ...
  const imageUrl = product.imageUrl || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800&h=600&fit=crop';
  ...
  const handleAddToCart = () => {
    ...
    const optionsArray: SelectedOption[] = Object.entries(selectedOptions).map(([name, choice]) => {
  ...
  const optionsPrice = Object.entries(selectedOptions).reduce((sum, [name, choice]) => {
    const option = product.options.find(opt => opt.name === name);
    const choiceObj = option?.choices.find(c => c.label === choice);
                      ...
                      <div className="space-y-2">
                        {option.choices.map((choice) => (
                          <label
                            ...
                            style={{
                              borderColor: selectedOptions[option.name] === choice.label ? '#f97316' : '#e5e7eb'
                            }}
                          >
                            <div className="flex items-center gap-3">
                              <input
                                ...
                                onChange={() => handleOptionChange(option.name, choice.label)}
                                className="text-orange-500 focus:ring-orange-500"
                              />
                              <span className="font-medium">{choice.label}</span>
                            </div>
                            {choice.price > 0 && (
                              <span className="text-gray-600">+{formatCurrency(choice.price)}</span>
                            )}
                          </label>
                        ))}
                      </div>
